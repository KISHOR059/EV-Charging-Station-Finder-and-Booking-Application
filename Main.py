...
def viewbooking():
         ...
         data1 = cur1.fetchall()
         conn11 = mysql.connector.connect(user='root', password='', host='localhost', database='lams1')
         # cursor = conn.cursor()
         cur11 = conn11.cursor()
         cur11.execute("SELECT SUM(amount) FROM booking where uname='"+uname+"' and status='Accept' ")
         data11 = cur11.fetchall()
         print(data11[0])


         return render_template("viewbooking.html",data=data1,data11=data11)
@app.route("/vrecharge")
def vrecharge():
         #categories=request.form['id']
         uname=session['uname']
